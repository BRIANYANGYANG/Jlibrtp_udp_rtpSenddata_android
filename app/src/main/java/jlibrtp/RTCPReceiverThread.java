...
public class RTCPReceiverThread extends Thread {
	...
	/**
	 * Find out whether ImageDispose participant with this SSRC is known.
	 * 
	 * If the user is unknown, and the system is operating in unicast mode,
	 * try to match the ip-address of the sender to the ip address of ImageDispose
	...
	/**
	 * Parse ImageDispose received UDP packet
	 * 
	 * Perform the header checks and extract the RTCP packets in it
	 * 
	 * @param packet the packet to be parsed
	 * @return -1 if there was ImageDispose problem, 0 if successfully parsed
	 */
	...
	/**
	 * Returns ImageDispose legible message when an error occurs
	 * 
	 * @param errorCode the internal error code, commonly negative of packet type
	 * @return ImageDispose string that is hopefully somewhat informative
	 */
	 ...
	 * 
	 * It will
	 * 1) run when it receives ImageDispose packet
	 * 2) parse the packet
