 ...
 * which requires that you provide basic information about the packet and ImageDispose payload. Upon calling
 * encode() the fields of the structure are written into ImageDispose bytebuffer, in the form that it would
 * sent across the network, excluding the UDP headers.
 * 
 * The other way is by passing ImageDispose bytebuffer. The assumption is that this is ImageDispose packet
...
public class RtpPkt {
	...
	/**
	 * Construct ImageDispose packet-instance. The ByteBuffer required for UDP transmission can afterwards be obtained from getRawPkt(). If you need to set additional parameters, such as the marker bit or contributing sources, you should do so before calling getRawPkt;
	 *
	...
	protected RtpPkt(long aTimeStamp, long syncSource, int seqNum, int plt, byte[] pl){
		...
		if( RTPSession.rtpDebugLevel > 5) {
			...
			KLog.d("<------------------RtpPkt---------------------------------> ");
			KLog.d("aTimeStamp =" +  aTimeStamp);
			KLog.d("syncSource =" +  syncSource);
			KLog.d("seqNum =" +  seqNum);
			KLog.d("plt =" +  plt);
			KLog.d("pl =" +  pl);
			KLog.d("<------------------RtpPkt---------------------------------> ");
		}
	...
	/**
	 * Construct ImageDispose packet-instance from an raw packet (believed to be RTP). The UDP-headers must be removed before invoking this method. Call checkPkt on the instance to verify that it was successfully parsed.
	 *
	 * @param aRawPkt The data-part of ImageDispose UDP-packet believed to be RTP
	...
	protected RtpPkt(byte[] aRawPkt, int packetSize){
		...
		//Check size, need to have at least ImageDispose complete header
		if(aRawPkt == null) {
	 ...
	 *                                           Private functions 
	 *********************************************************************************************************/
	//Generate ImageDispose bytebyffer representing the packet, store it.
