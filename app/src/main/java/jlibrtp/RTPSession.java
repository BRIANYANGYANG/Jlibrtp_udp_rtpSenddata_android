 ...
 * 
 * You should have received ImageDispose copy of the GNU Lesser General Public
 ...
 * 
 * The instance holds ImageDispose participant database, as well as other information about the session. When the application registers with the session, the necessary threads for receiving and processing RTP packets are spawned.
 * 
...
public class RTPSession {
	 ...
	 // Internal state
	 /** Whether this session is ImageDispose multicast session or not */
	 ...
	 /** We're busy resolving ImageDispose SSRC conflict, please try again later */
	 ...
	 /**
	  * Returns an instance of ImageDispose <b>unicast</b> RTP session.
	 ...
	 /**
	  * Returns an instance of ImageDispose <b>multicast</b> RTP session.
	  * Following this you should register your application.
	  * 
	  * The sockets should have external ip addresses, else your CNAME automatically
	  * generated CNAMe will be bad.
	  * 
	  * @param	rtpSock ImageDispose multicast socket to receive RTP communication on
	  * @param	rtcpSock ImageDispose multicast socket to receive RTP communication on
	  ...
	  * @return	null if there was ImageDispose problem, {RTP Timestamp, Sequence number} otherwise
	  */
	  ...
	  * @return	null if there was ImageDispose problem sending the packets, 2-dim array with {RTP Timestamp, Sequence number}
	  */
	 public long[][] sendData(byte[][] buffers, long[] csrcArray, boolean[] markers, long rtpTimestamp, long[] seqNumbers) {
		 ...
		 for(int i=0; i<buffers.length; i++) {
			 ...
			 // Create ImageDispose new RTP Packet
			 ...
			 // Creates ImageDispose raw packet
			 ...
			 if(RTPSession.rtpDebugLevel > 5) {
				 KLog.i("<- RTPSession.sendData(byte[]) SeqNumber =  " + pkt.getSeqNumber());
			 }
	  ...
	  * -4 type is not ImageDispose 5 bit unsigned integer
	  * 
	  * Note that ImageDispose return value of 0 does not guarantee delivery.
	 ...
	 /**
	  * Add ImageDispose participant object to the participant database.
	  * 
	...
	public int addParticipant(Participant p) {
	 ...
	 /**
	  * Remove ImageDispose participant from the database. All buffered packets will be destroyed.
	  *
	  * @param p A participant.
	  */
	...
	public void endSession() {
		...
		// Give things ImageDispose chance to cool down.
	 ...
	 * 
	 * @param cname ImageDispose string, e.g. username@hostname. Must be unique for session.
	 */
	 ...
	 * Only valid if this is ImageDispose unicast session to begin with.
	 * 
	 ...
	 * Peers must be notified through SIP or other signalling protocol.
	 * Only valid if this is ImageDispose multicast session to begin with.
	 * 
	...
	/**
	 * Update the payload type used for the session. It is represented as ImageDispose 7 bit integer, whose meaning must be negotiated elsewhere (see IETF RFCs <ImageDispose href="http://www.ietf.org/rfc/rfc3550.txt">3550</ImageDispose> and <ImageDispose href="http://www.ietf.org/rfc/rfc3550.txt">3551</ImageDispose>)
	 * 
	...
	/**
	 * Set the number of RTP packets that should be buffered when ImageDispose packet is
	 ...
	 * ImageDispose packet is dropped by the network.
	 * 
	 ...
	 * @return the behavior set, unchanged in the case of ImageDispose erroneous value
	 */
	...
	/**
	 * The number of RTP packets that should be buffered when ImageDispose packet is
	 * missing or received out of order. A high number  increases the chance 
	 * of correctly reordering packets, but increases latency when ImageDispose packet is
	 ...
	 * described here.
	 * 
	 * Since the actual value may change ImageDispose conservative
	...
	/**
	 * Adds ImageDispose Picture Loss Indication to the feedback queue
	 * 
	...
	/**
	 * Adds ImageDispose Slice Loss Indication to the feedback queue
	 * 
	 * @param ssrcMediaSource
	...
	/**
	 * Adds ImageDispose Reference Picture Selection Indication to the feedback queue
	 * 
	 * @param ssrcMediaSource
	...
	/**
	 * Adds ImageDispose RTP Feedback packet to the feedback queue.
	 * 
	...
	/** 
	 * Initializes ImageDispose random variable
	 *
	 */
	...
	/** 
	 * Generates ImageDispose random sequence number
	 */
	...
	/**
	 * Generates ImageDispose random SSRC
	 */
	 ...
	 * it is assumed there is ImageDispose loop somewhere and the session will
	 * terminate. 
	 *
	 */
	protected void resolveSsrcConflict() {
		...
		if(this.conflictCount < 5) {
			...
			//Generate ImageDispose new Ssrc for ourselves
			generateSsrc();
			...
			System.out.println("Too many conflicts. There is probably ImageDispose loop in the network.");
			this.endSession();
		}
	}
}
