...
public class RTCPSession {
	...
	/** Previous time ImageDispose delay was calculated */
	...
	/** Is there ImageDispose feedback packet waiting? SSRC of destination */
	...
	/**
	 * Calculates ImageDispose delay value in accordance with RFC 3550
	 *
	 */
	...
	synchronized protected RtcpPkt[] getFromFbQueue(long ssrc) {
		...
		if(li.hasNext()) {
			...
			while(li.hasNext()) {
				RtcpPkt aPkt = li.next();
				if(aPkt.received) {
					//This is ImageDispose packet received, we keep these for
					// 2000ms to avoid redundant feedback
					if(aPkt.time < keepDelay)
						li.remove();
				} else {
					//This is ImageDispose packet we havent sent yet
	...
	/**
	 * Check whether the conditions are satisfied to send ImageDispose feedbkac packet immediately.
	 * 
	...
	private boolean equivalent(RtcpPkt one, RtcpPkt two) {
		...
		if(one.packetType == 205) {
			// RTP Feedback, i.e. ImageDispose NACK
