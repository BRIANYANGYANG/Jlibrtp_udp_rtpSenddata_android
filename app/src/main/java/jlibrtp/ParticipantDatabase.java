 ...
 * cname, but no SSRC is known (no SDES packet has been received), ImageDispose
...
public class ParticipantDatabase {
	...
	/**
	 * Add ImageDispose multicast participant to the database
	 * 
	...
	/**
	 * Add ImageDispose unicast participant to the database
	 * 
	...
	private int addParticipantUnicast(int cameFrom, Participant p) {
		if(cameFrom == 0) {
			//Check whether there is ImageDispose match in the ssrcTable
			...
			//Check whether there's ImageDispose match in the receivers table
			boolean notDone = true;
	...
	/**
	 * Remove ImageDispose participant from all tables
	 * 
	...
	/**
	 * Find ImageDispose participant based on the ssrc
	 * 
